...
- ``analyze(hanzi, existing_tags)`` returns the data the Next.js layer
  ...
  the UI, and broad english semantic tags. We pull pinyin + definition
  from CC-CEDICT and radicals/components from the local makemeahanzi
  dictionary; only ``semantic_tags`` is generated by the LLM, and even
  there we bias toward the user's existing tag vocabulary so new words
  cluster naturally instead of inventing brand-new tags every time.
